...
func (b *sqlBase) buildCreateTableSQL(schema, table, key string, fields Vars) (string, error) {
	...
	for name, field := range fields {
		...
		if strings.EqualFold(col, key) {
			seenKey = true
			integerKey = isIntegerMigrateType(field.Type)
			if integerKey {
				def = migrateIdentityDefinition(d, col)
			}
		}
		if !integerKey || !strings.EqualFold(col, key) {
	...
	if !seenKey {
		integerKey = true
		cols = append([]string{migrateIdentityDefinition(d, key)}, cols...)
	}
	if !integerKey {
...
func isIntegerMigrateType(typ string) bool {
	switch strings.ToLower(strings.TrimSpace(typ)) {
	case "int", "integer", "int64", "uint", "uint64":
		return true
	default:
		return false
	}
...
func migrateIdentityDefinition(d Dialect, column string) string {
	quoted := d.Quote(column)
	dialect := strings.ToLower(strings.TrimSpace(d.Name()))
	switch {
	case dialect == "pgsql" || dialect == "postgres" || dialect == "postgresql":
		return quoted + " BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY"
	case strings.Contains(dialect, "mysql"):
		return quoted + " BIGINT AUTO_INCREMENT PRIMARY KEY"
	case strings.Contains(dialect, "sqlite"):
		return quoted + " INTEGER PRIMARY KEY"
	default:
		return quoted + " BIGINT PRIMARY KEY"
	}
}
